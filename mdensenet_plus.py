...
class MDenseNetBackbone(nn.Module):
    def __init__(self, in_channels, num_features, growth_rate, kernel_size, scale=(2,2), dilated=False, norm=True, nonlinear='relu', depth=None, out_channels=None, eps=EPS, num_bottleneck_layers=2, freq_bins=2049):
        """
        Args:
            in_channels <int>
            num_features <int>
        ...
        assert(_in_channels == _growth_rate)
        print(f'bottleneck: in {_in_channels}, growth {_growth_rate}')
        ...
        #     dilated=dilated[num_encoder_blocks], norm=norm[num_encoder_blocks], nonlinear=nonlinear[num_encoder_blocks], depth=depth[num_encoder_blocks]
        # )
        encoder_out_size = np.ceil(freq_bins / 2 ** num_encoder_blocks).astype(int)
        bottleneck = TransformerEncoderBottleneck(_in_channels, encoder_out_size, num_bottleneck_layers)
        ...
        if out_channels is not None:
            ...
            net = []
            norm2d = nn.BatchNorm2d(_in_channels, eps=eps)
            net.append(norm2d)
...
class TransformerEncoderBottleneck(nn.Module):
    def __init__(self, in_channels, num_features, n_layers = 2, d_feedforward = 128):
        super().__init__()

        encoder_layer = nn.TransformerEncoderLayer(d_model=in_channels*num_features, dim_feedforward=d_feedforward, nhead=in_channels, batch_first=True)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)

    def forward(self, input):
        x = input.reshape(input.size(0), -1, input.size(-1)).transpose(1,2)
        x = self.encoder(x)
        x = x.transpose(2,1).reshape(input.size())
